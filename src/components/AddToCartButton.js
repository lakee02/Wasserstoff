...
// Component for the shopping cart panel
const AddToCartButton = () => {
  const [totalPrice, setTotalPrice] = useState(0);
  const { cart, open } = useSelector((store) => store.app);
  const dispatch = useDispatch();

  // Function to toggle the visibility of the cart panel
  const setOpen = (para = false) => {
    dispatch(setOpenCart(para));
  };
  ...
  // Function to handle checkout, clear cart, and close the cart panel
  const checkoutItem = () => {
    if (cart.length === 0) {
  ...
  useEffect(() => {
    const getTotalPrice = () => {
      let totalAmount = cart.reduce((acc, element) => {
                              ...
                              <li key={product.id} className="flex py-6">
                                ...
                                <div className="ml-4 flex flex-1 flex-col">
                                  ...
                                  <div className="flex flex-1 items-end justify-between text-sm">
                                    <div className="flex text-lg bg-gray-200 px-2">
                                      {/* Decrement quantity */}
                                      <span
